...
def launch_interface_v2():
    ...
    query = """
        SELECT 
            ...
            t.start_seconds AS [TotalSeconds],
            t.artist AS [Artist], 
    ...
    print("Pulling index-optimized music cache...")
    df = pd.read_sql(query, con=engine)
    
    if df.empty:
        print("Error: The database contains no records. Please run import_spreadsheet.py first.")
        return

    total_tracks = len(df)
    total_mixes = df['Episode'].nunique()

    # DYNAMIC DURATION CALCULATION ENGINE
    df['NextSeconds'] = df.groupby('Episode')['TotalSeconds'].shift(-1)
    df['DurationSeconds'] = df['NextSeconds'] - df['TotalSeconds']

    # Clean top artists leaderboard
    ...
    def format_duration(secs):
        ...
        dmins = int(secs) // 60
        dsecs = int(secs) % 60
        return f"{dmins}:{dsecs:02d}"

    def format_start_time(secs):
        if secs <= 0 or pd.isna(secs):
            return '--:--'
        hours = int(secs) // 3600
    ...
    df['Duration'] = df['DurationSeconds'].apply(format_duration)
    df['StartTimeStr'] = df['TotalSeconds'].apply(format_start_time)

    def make_button(row):
        url = row['BaseURL']
        seconds = int(row['TotalSeconds'])
        time_str = row['StartTimeStr']
        ...
        btn_label = f" @ {time_str}" if time_str != '--:--' else ""
        is_unknown_start = (seconds == 0 and dur_str == '--:--')
        ...
        if is_breaking_special:
            ...
            if not is_unknown_start:
                mins = seconds // 60
                secs = seconds % 60
                time_url = f"{clean_url}#t={mins}m{secs}s"
                return f'<a href="{time_url}" target="_blank" class="btn btn-sm btn-orange text-nowrap d-inline-flex align-items-center justify-content-center gap-1">🌐 Play{btn_label}</a>'
            return f'<a href="{clean_url}" target="_blank" class="btn btn-sm btn-outline-secondary text-nowrap d-inline-flex align-items-center justify-content-center gap-1">🌐 Open Mix</a>'
            
        if is_unknown_start:
            return f'<button onclick="loadTrack(\'{url}\', 0)" class="btn btn-sm btn-outline-secondary text-nowrap">▶ Play Mix</button>'
        return f'<button onclick="loadTrack(\'{url}\', {seconds})" class="btn btn-sm btn-orange text-nowrap">▶ Play{btn_label}</button>'
    ...
    def format_episode_cell(row):
        ...
        return f'<div class="episode-container-inner"><div class="episode-title-cell text-truncate" title="{ep_name}">{ep_name}</div><button onclick="copyTracklist(this, \'{ep_name.replace("'", "\\'")}\')" class="btn btn-link btn-copy-icon p-0 ms-2" title="Copy Tracklist">📋</button></div>'
        
    df['Episode'] = df.apply(format_episode_cell, axis=1)
    
    df_display = df[['Episode', 'Air Date', 'Track #', 'Duration', 'Artist', 'Track Title', 'Record Label', 'Listen']]
    
    # CRITICAL FIX: Ensure escape=False is forced here so custom HTML blocks render as proper interactive DOM elements
    table_html = df_display.to_html(escape=False, index=False, classes="table align-middle")

    styling = """<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Uplifting Only Vault Console v2</title>
    ...
    <style>
        ...
        .col-ep { width: 22% !important; text-align: left !important; overflow: hidden; }
        ...
        .col-label { width: 9% !important; text-align: left !important; }
        .col-listen { width: 13% !important; text-align: center !important; }
        
        .episode-container-inner { display: flex; align-items: center; justify-content: space-between; overflow: hidden; width: 100%; }
        .episode-title-cell { font-weight: 600; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; flex-grow: 1; color: #0f172a; max-width: 82%; }
        ...
        .btn-orange { background-color: #ff5500; color: white; font-weight: 500; border: none; text-decoration: none; display: inline-block; width: 100%; text-align: center; white-space: nowrap !important; }
        .btn-orange:hover { background-color: #e04b00; color: white; }
        .btn-outline-secondary { width: 100%; text-align: center; background-color: #ffffff; white-space: nowrap !important; }
...
<div class="container-fluid vault-card">
    ...
    <div class="row g-2 console-row">
        <div class="col-md-5 d-flex flex-column justify-content-between py-1">
            <div>
                <h2 class="m-0 p-0" style="line-height: 1.1; font-size: 1.6rem;">Uplifting Only Vault Console <span class="text-muted" style="font-size: 1rem;">v2.0</span></h2>
                <div class="mt-2">
                    <span class="badge bg-dark rounded-pill">__MIXES__ Mixes</span>
...
<script>
    ...
    function isPlaceholderMatch(cellText, queryWord) {
        let cleanCellText = cellText.toLowerCase().trim();
        let cleanQuery = queryWord.toLowerCase().trim();
        
        if (cleanQuery === 'id') {
            let tokens = cleanCellText.split(/[\s,;&\(\)\[\]\-]+/);
            return tokens.includes('id') || cleanCellText === 'id';
        }
        return cleanCellText.includes(cleanQuery);
    }
    ...
    function filterRows(value) {
        ...
        window.requestAnimationFrame(() => {
            for (let i = 0; i < rows.length; i++) {
                ...
                let rowArtist = row.cells[4] ? row.cells[4].textContent.toLowerCase().trim() : '';
                let rowTitle = row.cells[5] ? row.cells[5].textContent.toLowerCase().trim() : '';
                
                if (query.includes(' - ')) {
                    let parts = query.split(' - ');
                    let artistQuery = parts[0].trim();
                    let titleQuery = parts[1].trim();
                    
                    let artistMatch = isPlaceholderMatch(rowArtist, artistQuery);
                    let titleMatch = isPlaceholderMatch(rowTitle, titleQuery);
                    
                    row.style.display = (artistMatch && titleMatch) ? '' : 'none';
                } else {
                    if (query === 'id') {
                        row.style.display = (isPlaceholderMatch(rowArtist, 'id') || isPlaceholderMatch(rowTitle, 'id')) ? '' : 'none';
                    } else {
    ...
    function playRandomTrack() {
        ...
        // CRITICAL FIX: Specific index target for the "Listen" column cell structure
        let listenCell = randomRow.cells[7];
        if (listenCell) {
            let targetAction = listenCell.querySelector('a, button');
            if (targetAction) {
    ...
    function copyTracklist(btnElement, targetEpisode) {
        ...
        navigator.clipboard.writeText(fullText).then(() => {
            ...
            console.log('Failed to copy playlist: ', err);
        });
    }
    ...
    html_document = (styling + table_html + js_controls).replace("__VERSION__", SCRIPT_VERSION) \
    ...
    print(f"Dashboard Build v{SCRIPT_VERSION} compiled. Synchronized directly to {HTML_OUTPUT}")
